document.addEventListener("DOMContentLoaded", async () => {
  ...
  const timeElapse = (date) => {
    ...
    const result = `Дней <span class="digit">${days}</span> Часов <span class="digit">${hours}</span> Минут <span class="digit">${minutes}</span>Секунд <span class="digit">${seconds}</span>`;
